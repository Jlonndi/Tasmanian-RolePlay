...
AddEventHandler('fuel:startFuelUpTick', function(pumpObject, ped, vehicle)
	...
	while isFueling do
		...
		if not pumpObject then
			...
			if GetAmmoInPedWeapon(ped, 883325847) - fuelToAdd * 10 >= 0 then

				currentFuel = oldFuel + fuelToAdd



				SetPedAmmo(ped, 883325847, math.floor(GetAmmoInPedWeapon(ped, 883325847) - fuelToAdd * 10))
	...
	if pumpObject then
		...
		TriggerServerEvent('LegacyFuel:PayFuel', currentCost)
				...
				if DoesEntityExist(vehicle) and GetDistanceBetweenCoords(GetEntityCoords(ped), vehicleCoords) < 2.5 then
					...
					if not DoesEntityExist(GetPedInVehicleSeat(vehicle, -1)) then
						...
						if GetSelectedPedWeapon(ped) == 883325847 then
							...
							if GetAmmoInPedWeapon(ped, 883325847) < 10 then
	...
	Citizen.CreateThread(function()
		...
		while true do
			...
			if displayHud then
				...
				DrawAdvancedText(0.174 - x, 0.77 - y, 0.0090, 0.0628, 0.6, kmh, 255, 255, 255, 255, 6, 1)

				DrawAdvancedText(0.2195 - x, 0.77 - y, 0.0090, 0.0628, 0.6, fuel, 255, 255, 255, 255, 6, 1)

				DrawAdvancedText(0.148 - x, 0.7765 - y, 0.0090, 0.0628, 0.4, "KPH                 FUEL", 255, 255, 255, 255, 6, 1)
