...
AddEventHandler('esx_vehicleshop:setVehicleOwned', function(vehicleProps)
	...
	TriggerClientEvent('trp-core:PDMLOGS1', source, vehicleProps.plate)
	MySQL.Async.execute('INSERT INTO owned_vehicles (owner, plate, vehicle) VALUES (@owner, @plate, @vehicle)',
	{
		...
		TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type = 'success', text = _U('vehicle_belongs', vehicleProps.plate), length = 10000, style = { ['background-color'] = '#09d940', ['color'] = '#FFFFFF' } })
	end)
end)
...
AddEventHandler('esx_vehicleshop:setVehicleOwnedPlayerId', function(playerId, vehicleProps)
	...
	TriggerClientEvent('trp-core:PDMLOGS1', playerId, vehicleProps.plate)
	MySQL.Async.execute('INSERT INTO owned_vehicles (owner, plate, vehicle) VALUES (@owner, @plate, @vehicle)',
	{
		['@owner']   = xPlayer.identifier,
		['@plate']   = vehicleProps.plate,
...
ESX.RegisterServerCallback('esx_vehicleshop:buyVehicle', function(source, cb, vehicleModel)
	...
	if vehicleData and xPlayer.getInventoryItem('money').count >= vehicleData.price then
		xPlayer.removeInventoryItem('money', vehicleData.price)
		TriggerClientEvent('trp-core:PDMLOGS', source, vehicleModel, vehicleData.price)
		cb(true)
	else
		cb(false)
	end
					...
					if result[1] then -- does the owner match?
						...
						if vehicle.model == model then
							if vehicle.plate == plate then
								xPlayer.addInventoryItem('money', resellPrice)
								-- add triggerevent(plate,resellprice logs so it logs sales...)
								RemoveOwnedVehicle(plate)
								cb(true)
							else
