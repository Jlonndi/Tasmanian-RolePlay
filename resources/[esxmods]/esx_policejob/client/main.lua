...
function setUniform(uniform, playerPed)
	TriggerEvent('skinchanger:getSkin', function(skin)
		...
		if uniformObject then
			...
			if uniform == 'jacket' then
				SetPedArmour(playerPed, 100)
			end
			if uniform == 'superivsor' then
				SetPedArmour(playerPed, 100)
			end
			if uniform == 'recruit' then
				SetPedArmour(playerPed, 100)
			end
			if uniform == 'constable' then
				SetPedArmour(playerPed, 100)
			end
			if uniform == 'first_class_const' then
				SetPedArmour(playerPed, 100)
			end
			if uniform == 'senior_const' then
				SetPedArmour(playerPed, 100)
			end
			if uniform == 'leading_senior_const' then
				SetPedArmour(playerPed, 100)
			end
			if uniform == 'sergeant' then
				SetPedArmour(playerPed, 100)
			end
			if uniform == 'senior_seargeant' then
				SetPedArmour(playerPed, 100)
			end
			if uniform == 'second_senior_sergeant' then
				SetPedArmour(playerPed, 100)
			end
			if uniform == 'inspector' then
				SetPedArmour(playerPed, 100)
			end
			if uniform == 'senior_inspector' then
				SetPedArmour(playerPed, 100)
			end
			if uniform == 'superintendant' then
				SetPedArmour(playerPed, 100)
			end
			if uniform == 'chief_superintendant' then
				SetPedArmour(playerPed, 100)
			end
			if uniform == 'assistant_comissioner' then
				SetPedArmour(playerPed, 100)
			end
			if uniform == 'deputy_commissioner' then
				SetPedArmour(playerPed, 100)
			end
			if uniform == 'commissioner' then
				SetPedArmour(playerPed, 100)
			end
		else
...
function OpenCloakroomMenu()
	...
	local elements = {
		...
		{label = _U('supervisor'), uniform = 'supervisor'}
	}
				...
				if not isInService then
					...
					ESX.TriggerServerCallback('esx_service:enableService', function(canTakeService, maxInService, inServiceCount)
						if not canTakeService then
							...
							TriggerServerEvent('esx_service:notifyAllInService', notification, 'police')
							TriggerEvent('esx_policejob:PTTRadio')
							TriggerEvent('esx_policejob:updateBlip')
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions', {
		...
		if data.current.value == 'citizen_interaction' then
			local elements = {
				...
				{label = 'Hard Cuff',   value = 'ruskicuff'},
...
RegisterNetEvent('esx_policejob:PTTRadio')
AddEventHandler('esx_policejob:PTTRadio', function()
	if ESX.PlayerData.job.name == 'police' then

		local holstered = true
		local holsteredtaser = true
				...
				if DoesEntityExist( ped ) and not IsEntityDead( ped ) and not IsPedInAnyVehicle(PlayerPedId(), true) and CheckSkin(ped) then
					...
					if CheckWeapon(ped) then
						if holstered then
						 ...
						 -- GUN OUT // 3 is leg hoslter, 2 is hip, 120 is front
						if holsterStyle == "hip" then
							SetPedComponentVariation(ped, 7, 2, 0, 0)
						elseif holsterStyle == "leg" then
					...
					elseif not CheckWeapon(ped) then
						if not holstered then
						 ...
						 -- GUN IN HOLSTER // 1 is leg holster, 8 is hiAp, 119 is front
						if holsterStyle == "hip" then
		...
		Citizen.CreateThread(function()
			while (ESX.PlayerData.job.name == 'police') do
				...
				if DoesEntityExist( ped ) and not IsEntityDead( ped ) and not IsPedInAnyVehicle(PlayerPedId(), true) and CheckSkin(ped) then
					loadAnimDict( "rcmjosh4" )
					loadAnimDict( "weapons@pistol@" )
						if HasPedGotWeapon(ped, (GetHashKey("WEAPON_STUNGUN"))) then
							if holsteredtaser then
								SetPedComponentVariation(ped, 10, 8, 0, 0)
							end
							if CheckTaser(ped) then
								if holsteredtaser then
									TaskPlayAnim(ped, "weapons@pistol@", "holster_2_aim", 8.0, 2.0, -1, 48, 10, 0, 0, 0 )
									Citizen.Wait(600)
									ClearPedTasks(ped)
									holsteredtaser = false
									SetPedComponentVariation(ped, 10, 9, 0, 0)
								end
							elseif not CheckTaser(ped) then
								if not holsteredtaser then
									TaskPlayAnim(ped, "weapons@pistol@", "aim_2_holster", 8.0, 2.0, -1, 48, 10, 0, 0, 0 )
									Citizen.Wait(500)
									ClearPedTasks(ped)
									holsteredtaser = true
		...
		function CheckTaser(ped)
			for i = 1, #taser do
				if GetHashKey(taser[i]) == GetSelectedPedWeapon(ped) then
					return true
				end
			end
			return false
...
RegisterNetEvent('TRP:FineMenu')
AddEventHandler('TRP:FineMenu', function(closetplayer)
	OpenFineMenu(closetplayer)
end)
...
RegisterNetEvent('TRP:CUFF')
AddEventHandler('TRP:CUFF', function()
	local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
				if closestPlayer ~= -1 and closestDistance <= 2.0 then
					
					
						TriggerServerEvent('esx_ruski_areszt:startAreszt', GetPlayerServerId(closestPlayer)) -- Rozpoczyna Funkcje na Animacje (Cala Funkcja jest Powyzej^^^)
						exports['progressBars']:startUI(1800, "Cuffing Suspekt")
						TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 2.0, 'cuff', 0.7)
						Citizen.Wait(1800)
...
RegisterNetEvent('TRP:CUFF1')
AddEventHandler('TRP:CUFF1', function()
	local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
				if closestPlayer ~= -1 and closestDistance <= 2.0 then
					
					
						TriggerServerEvent('esx_ruski_areszt:startAreszt', GetPlayerServerId(closestPlayer)) -- Rozpoczyna Funkcje na Animacje (Cala Funkcja jest Powyzej^^^)
						exports['progressBars']:startUI(500, "Restraining Patient")
						--TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 2.0, 'cuff', 0.7)
						Citizen.Wait(500)
...
RegisterNetEvent('TRP:DRAG')
AddEventHandler('TRP:DRAG', function()
	local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
	if closestPlayer ~= -1 and closestDistance <= 2.0 then
	TriggerServerEvent('esx_policejob:drag', GetPlayerServerId(closestPlayer))
...
RegisterNetEvent('TRP:PUTVEHICLE')
AddEventHandler('TRP:PUTVEHICLE', function()
	local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
	if closestPlayer ~= -1 and closestDistance <= 2.0 then
	TriggerServerEvent('esx_policejob:putInVehicle', GetPlayerServerId(closestPlayer))
	end
end)
RegisterNetEvent('TRP:PULLVEHICLE')
AddEventHandler('TRP:PULLVEHICLE', function()
	local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
	if closestPlayer ~= -1 and closestDistance <= 2.0 then
		TriggerServerEvent('esx_policejob:OutVehicle', GetPlayerServerId(closestPlayer))
	end
...
RegisterNetEvent('trp:rob')
AddEventHandler('trp:rob', function(robber)
	if handsup then
		TriggerEvent('TRP:INVENTORYOPENROBINVENTORY', robber)
	else if isHandcuffed then
		TriggerEvent('TRP:INVENTORYOPENROBINVENTORY', robber)
	else 
		Print('FuckoffDickHead')
	end
end
end)
