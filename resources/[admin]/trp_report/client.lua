...
RegisterNetEvent('trp_rilessucks:report')
AddEventHandler('trp_rilessucks:report', function(text)
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'report', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'report_confirm', {
			...
			if action == 1 then
				TriggerEvent('trp_rilessucks:report', data.value)
...
--[[RegisterCommand("openpanel", function(source, args, rawCommand)

TriggerEvent('trp_rilessucks:openReports')
end, false)]]
RegisterNetEvent('trp_rilessucks:openReports')
AddEventHandler('trp_rilessucks:openReports', function()
	if not admin then
		openReports2()
	end
end)
...
RegisterNetEvent('trp_rilessucks:notifyStaff')
AddEventHandler('trp_rilessucks:notifyStaff', function(text)
