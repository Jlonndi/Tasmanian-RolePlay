...
TriggerEvent('es:addCommand', 'report', function(source, args, user)
	TriggerClientEvent('reportUI:openGui', source)
end, {help = 'Report an player, incident or something else.'})
TriggerEvent('es:addGroupCommand', 'openpanel', 'mod', function(source, args, user)
	TriggerClientEvent('trp_rilessucks:openReports', source)
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Insufficient Permissions.' } })
end, {help = 'Opens Report Panel for Admins'})
		...
		for k,playerId in ipairs(players) do
			...
			if xPlayer then
				...
				if group == 'mod' or group == 'admin' or group == 'superadmin' then
					TriggerClientEvent('trp_rilessucks:notifyStaff', playerId, notifyMessage) 
				end
			end
		end
