isOnDuty = false
RegisterNetEvent('isonduty')
AddEventHandler('isonduty', function()
  isOnDuty = true
end)
RegisterNetEvent('isOffDuty')
AddEventHandler('isOffDuty', function()
  isOnDuty = false
end)
RegisterCommand('noclip', function()
	if (isOnDuty == true) then
--		TriggerEvent('TRP:newPed')
		Citizen.Wait(150)-- 150ms for it to slow the fuck down so it pulls ID
		ToggleNoClipMode()
		
		notification('success', '<p>No-Clip Toggled</p>')
	end
end, false)

RegisterKeyMapping('noclip', 'Admin No-Clip', 'keyboard', 'F9')
--[[
