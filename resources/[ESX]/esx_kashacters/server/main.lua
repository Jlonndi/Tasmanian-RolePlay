...
local IdentifierTables = {
    ...
    --{table = "addon_account_data", column = "owner"},
    --{table = "addon_account_items", column = "owner"},
    {table = "datastore_data", column = "owner"},
    {table = "user_inventory", column = "identifier"},
    {table = "transactions", column = "identifier"},
    {table = "trp_usedcardealer", column = "owner"},
...
AddEventHandler('kashactersS:CharacterChosen', function(charid, ischar)
	...
	if type(charid) == "number" and type(ischar) == "boolean" then
        ...
        TriggerClientEvent('trp:ScreenshotAC', src, src, 'Hacking SQL Injection Attempt KASH#44 [Perma]')
        Citizen.Wait(10000)
        print('oh he fucked up')
        --TriggerEvent("el_bwh:ban", ESX.GetPlayerFromId(src), ESX.GetPlayerFromId(src), 'Hacking SQL Injection Attempt KASH#43 [Perma]', '2999-1-1 00:00:00', nil)
    end
end)
