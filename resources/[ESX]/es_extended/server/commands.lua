...
TriggerEvent('es:addGroupCommand', 'car', 'mod', function(source, args, user)
	TriggerClientEvent('esx:spawnVehicleAdmin', source, args[1])
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Insufficient Permissions.' } })
end, {help = _U('spawn_car'), params = {{name = "car", reason = 'reason', help = _U('spawn_car_param')}}})

TriggerEvent('es:addGroupCommand', 'cardel', 'admin', function(source, args, user)
	TriggerClientEvent('TRP:DV', source)
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Insufficient Permissions.' } })
...
--[[TriggerEvent('es:addGroupCommand', 'setmoney', 'admin', function(source, args, user)
	...
	if target and money_type and money_amount and xPlayer ~= nil then
		if money_type == 'cash' then
			xPlayer.addInventoryItem('money', money_amount)
		elseif money_type == 'bank' then
			xPlayer.addInventoryItem('money', money_amount)
		elseif money_type == 'black' then
			xPlayer.addInventoryItem('black_money', money_amount)
		else
