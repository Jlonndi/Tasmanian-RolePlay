...
$(function() {
    ...
    $(document).on("click", "#player", function() {
        copyText("https://steamcommunity.com/profiles/" + hexidtodec($(this).data("steamid")), "Steam Profile Link Copied to Clipboard", 1850);
    });
...
window.addEventListener('message', function(event) {
    switch (event.data.type) {
        case "toggle":
            ...
            break;
        case "setup":
            $(".playercount").text(event.data.pc);// counts players
        ...
        case "update":
            ...
            if (["list", "con", "disc"].includes(page)) {
                ...
                switch (page) {
                    case "list":
                        $(event.data.data.pd).each(function(k, v) { // sort here potentially
                            var pc = pingColor(v.ping);
                            newtable +=
                                "<tr id='player' data-steamid='" + v.stid + "' data-sid='" + v.sid.toString() + "'>" +
                                "<td>" + v.sid.toString() + "</td>" +
                                "<td" + (v.stid == mysteamid ? " style='color:cyan;'" : "") + ">" + v.name + (v.stid == mysteamid ? " " : "") + "</td>" +
...
document.onkeydown = function(data) {
    if (data.which == 27 || data.which == 46) {
