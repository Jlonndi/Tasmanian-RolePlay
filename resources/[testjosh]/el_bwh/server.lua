ESX = nil
local discord_webhook = "https://discord.com/api/webhooks/766085809438195743/PcLu4nVBUVbYf0FTfTyGogGlNitGI2xLERhKXcu9L8XAjHyHYQYDwfoNA208Tb4WsbXF" -- paste your discord webhook between the quotes if you want to enable discord logs.
local discord_webhook1 = "https://discord.com/api/webhooks/825593541576359966/OECIiQxh46PIMtrCySlF4iYCeoGqtZNNVqmtPOPeZFpWMKeqqdbC7qu5OvJWCE1gakMw"
...
Citizen.CreateThread(function() -- startup
    ...
    --sendToDiscord("el_bwh has been started...") PENIS
    ...
    ESX.RegisterServerCallback("trp_A:getPlayers",function(source,cb)
        local xPlayer = ESX.GetPlayerFromId(source)
        cb(ESX.GetPlayers())
    end)
end)
...
AddEventHandler("playerConnecting",function(name, setKick, def)
    ...
    if #identifiers>0 and identifiers[1]~=nil then
        ...
        if banned then
            print(("[^1"..GetCurrentResourceName().."^7] Banned player %s (%s) tried to join, their ban expires on %s (Ban ID: #%s)"):format(GetPlayerName(source),data.receiver[1],data.length and os.date("%d-%m-%Y %H:%M",data.length) or "PERMANENT",data.id))
            local kickmsg = Config.banformat:format(data.reason,data.length and os.date("%d-%m-%Y %H:%M",data.length) or "PERMANENT",' ',data.id)
           local playname = GetPlayerName(source)
            local biggay = ''..playname..' tried to connect whilst banned.'
            sendToDiscord1(kickmsg, biggay)
...
function sendToDiscord1(msg, PlayerName)
    if discord_webhook1 ~= "" then
        PerformHttpRequest(discord_webhook1, function(a,b,c)end, "POST", json.encode({embeds={{title=PlayerName,description=msg:gsub("%^%d",""),color=65280,}}}), {["Content-Type"]="application/json"})
    end
end
...
function logAdmin(msg)
    for k,v in ipairs(ESX.GetPlayers()) do
       ...
       --     TriggerClientEvent("chat:addMessage",v,{color={255,0,0},multiline=false,args={"BWH",msg}}) only for admins anyway?
            sendToDiscord(msg)
        end
    end
end
...
function banPlayer(xPlayer,xTarget,reason,length,offline)
    ...
    MySQL.Async.execute("INSERT INTO bwh_bans(id,receiver,sender,length,reason) VALUES(NULL,@receiver,@sender,@length,@reason)",{["@receiver"]=json.encode(targetidentifiers),["@sender"]=xPlayer.identifier,["@length"]=length,["@reason"]=reason},function(_)
        ...
        table.insert(bancache,{id=banid==nil and "1" or banid,sender=xPlayer.identifier,reason=reason,sender_name='TRP CORE',receiver=targetidentifiers,length=length})
        ...
        if xTarget then
            ...
            Citizen.SetTimeout(5000, function()
                DropPlayer(xTarget.source,Config.banformat:format(reason,length~=nil and timestring or "PERMANENT",'TRP CORE',banid==nil and "1" or banid))
            end)
