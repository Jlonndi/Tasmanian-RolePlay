...
-- Keep the same name as in the configt.lua
...
Citizen.CreateThread(function()
	while true do
		...
		for zone, data in pairs(configt.zones) do
...
Citizen.CreateThread(function()
    while true do
        ...
        if CurrentAction ~= nil then
            ...
            local coords = {
	    		x = configt.point[CurrentAction].x,
	    		y = configt.point[CurrentAction].y,
	    		z = configt.point[CurrentAction].z
	    	}
            
            -- Test if E key is pushed (released actually, to avoid possible spam)
            if IsControlJustReleased(0, configt.actionKey) then            	
            	ESX.Game.Teleport(playerPed, coords)
            	CurrentAction = nil
            else
            	-- Test if zone is in automatic teleport list
	            for i=1, #configt.auto, 1 do
	            	if configt.auto[i] == CurrentAction then
		            	ESX.Game.Teleport(playerPed, coords)
		            	CurrentAction = nil
	            	end
	            end
