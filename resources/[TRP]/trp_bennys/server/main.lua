...
AddEventHandler('esx_lscustom2:refreshOwnedVehicle', function(vehicleProps)
	...
	MySQL.Async.fetchAll('SELECT vehicle FROM owned_vehicles WHERE plate = @plate', {
		...
		if result[1] then
			...
			if vehicleProps.model == vehicle.model then
				...
				print(('esx_lscustom2: %s attempted to upgrade vehicle with mismatching vehicle model!'):format(xPlayer.identifier))
			end
		end
	end)
end)
