...
AddEventHandler('esx_ambulancejob:revive', function(target)
	...
	if xPlayer.job.name == 'ambulance' then
		...
		TriggerClientEvent('TRP:AMBULANCEJOB', target)
		--TriggerClientEvent('mythic_hospital:client:FieldTreatLimbs', target)
	else
...
AddEventHandler('esx_ambulancejob:heal', function(target, type)
	...
	if xPlayer.job.name == 'ambulance' then
		...
		TriggerClientEvent('TRP:AMBULANCEJOBHEAL', target)
...
RegisterServerEvent('esx_ambulancejob:splint')
AddEventHandler('esx_ambulancejob:splint', function(target, type)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if xPlayer.job.name == 'ambulance' then
		...
		print(type)
			TriggerClientEvent('TRP:AMBULANCEJOBSPLINT', target)
...
ESX.RegisterServerCallback('esx_ambulancejob:removeItemsAfterRPDeath', function(source, cb)
	...
	if Config.RemoveItemsAfterRPDeath then
		for i=1, #xPlayer.inventory, 1 do
			if xPlayer.inventory[i].count > 0 then
				TriggerClientEvent('TRP_CORE:DeathTax', xPlayer.source, xPlayer.inventory[i].name, xPlayer.inventory[i].count)
				Citizen.Wait(1000)
	...
	if Config.RemoveWeaponsAfterRPDeath then
		for i=1, #xPlayer.loadout, 1 do
			TriggerClientEvent('TRP_CORE:DeathTax', xPlayer.source, xPlayer.loadout[i].name, 1)
				Citizen.Wait(1000)
...
ESX.RegisterServerCallback('esx_ambulancejob:buyJobVehicle', function(source, cb, vehicleProps, type)
	...
	if price == 0 then
		...
		if xPlayer.getInventoryItem('money') >= price then
...
AddEventHandler('esx_ambulancejob:removeItem', function(item)
	...
	elseif item == 'medikit1' then
		TriggerClientEvent('esx:showNotification', _source, _U('used_splint'))
	end
end)
...
AddEventHandler('esx_ambulancejob:giveItem', function(itemName)
	...
	if xPlayer.job.name ~= 'ambulance' then
		...
		return
	elseif (itemName ~= 'medikit' and itemName ~= 'bandage' and itemName ~= 'medikit1' and itemName ~= 'medicalrestraints') then
		print(('esx_ambulancejob: %s attempted to spawn in an item!'):format(xPlayer.identifier))
		return
	end
...
TriggerEvent('es:addGroupCommand', 'revive', 'mod', function(source, args, user)
	if args[1] ~= nil then
		if GetPlayerName(tonumber(args[1])) ~= nil then
			print(('TRP: %s used admin revive'):format(GetPlayerIdentifiers(source)[1]))
			--TriggerServerEvent('TRP:core:anticheat', xPlayer.identifier, 'trp_ambulancejob: %s attempted to revive!')
		--	TriggerClientEvent('esx_ambulancejob:revive', tonumber(args[1]))
			TriggerClientEvent('TRP:AMBULANCEJOB', tonumber(args[1]))
		end
	else
		TriggerClientEvent('TRP:AMBULANCEJOB', source)
	end
