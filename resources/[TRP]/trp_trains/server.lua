...
RegisterServerEvent("TRP-Trains:PlayerSpawned")
AddEventHandler('TRP-Trains:PlayerSpawned', function()
...
AddEventHandler('playerDropped', function()
	...
	if trainHost == _source then -- If the host of the train is the source, the train disapear with him.
		...
		if Debug then print(('TRP-Trains: Player %s with id %i has quit. Looking for another host for the trains...'):format(name,_source)) end
		trainspawned = false
		trainHost = nil
...
function ChooseRandomPlayer(leaver)
	...
	if not hostfound then
		if Debug then print('TRP-Trains: No host are available for the trains. Waiting for another player to join the server...') end
	end
end
...
function SpawnTrain(player)
	if not trainHost then -- If we have a host for the train, trainHost should be not nil
		if not trainspawned then -- new session or the host has leave ?
			...
			if Debug then print(('TRP-Trains: Player %s with id %i will be the host for the trains'):format(name,player)) end
