...
RegisterNetEvent('trp:robbery:onPedDeath')
AddEventHandler('trp:robbery:onPedDeath', function(store)
    SetEntityHealth(peds[store], 0)
end)

RegisterNetEvent('trp:robbery:msgPolice')
AddEventHandler('trp:robbery:msgPolice', function(store, robber)
...
RegisterNetEvent('trp:robbery:robberyOver')
AddEventHandler('trp:robbery:robberyOver', function()
    robbing = false
end)

RegisterNetEvent('trp:robbery:talk')
AddEventHandler('trp:robbery:talk', function(store, text, time)
    robbing = false
...
RegisterNetEvent('trp:robbery:rob')
AddEventHandler('trp:robbery:rob', function(i)
        ...
        if not IsPedDeadOrDying(peds[i]) then
            ...
            if not IsPedDeadOrDying(peds[i]) then
                ...
                while timer >= GetGameTimer() do
                    ...
                    if IsPedDeadOrDying(peds[i]) then -- ped dies cancels robbery
...
--if robbing23 then
                 --       Wait(0)
                        local biggayy = GetPlayerServerId(-1)

                        ESX.TriggerServerCallback("trp:core:getKeyRobbery", function(authkey)

                           local youwontgetthiskey = authkey
                
                                PlaySoundFrontend(-1, 'ROBBERY_MONEY_TOTAL', 'HUD_FRONTEND_CUSTOM_SOUNDSET', true)
                          
                               -- print('why is this serverwide')
                                
                                TriggerServerEvent('trp:robbery:pickUp', i, authkey, biggayy)
...
RegisterNetEvent('trp:robbery:resetStore')
AddEventHandler('trp:robbery:resetStore', function(i)
...
Citizen.CreateThread(function()
    ...
    Citizen.CreateThread(function()
        while true do
            for i = 1, #peds do
                if IsPedDeadOrDying(peds[i]) then
                    TriggerServerEvent('trp:robbery:pedDead', i)
                end
            end
            Wait(5000)
        end
            ...
            if IsPlayerFreeAiming(PlayerId()) then
                for i = 1, #peds do
                    if HasEntityClearLosToEntityInFront(me, peds[i], 19) and not IsPedDeadOrDying(peds[i]) and GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 5.0 then
                        if not robbing then
                            local canRob = nil
                            ESX.TriggerServerCallback('trp:robbery:canRob', function(cb)
                                canRob = cb
                            end, i)
                            while canRob == nil do
                                Wait(0)
                            ...
                            if canRob == true then
                                ...
                                while scared < 100 and not IsPedDeadOrDying(peds[i]) and GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 7.5 do
                                    ...
                                    if IsPedArmed(me, 7) and GetAmmoInClip(me, GetSelectedPedWeapon(me)) > 0 and IsControlPressed(0, 24) then
                                        sleep = 50
                                ...
                                if GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 7.5 then
                                    if not IsPedDeadOrDying(peds[i]) then
                                        TriggerServerEvent('trp:robbery:rob', i)
                                        robbing23 = true
                                        while robbing do Wait(0) if IsPedDeadOrDying(peds[i]) then robbing = false end end
                                    end
                                else
                            ...
                            elseif canRob == 'no_cops' then
                                ...
                                TriggerEvent('trp:robbery:talk', i, '~g~*' .. Translation[Config.Locale]['shopkeeper'] .. '* ~w~' .. Translation[Config.Locale]['robbed'], 5)
                                Wait(2500)
                            end
                        end
                    end
