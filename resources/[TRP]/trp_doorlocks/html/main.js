$('document').ready(function() {
    ...
    document.onkeydown = function (data) {
        if (data.which == 27) {
            $.post('https://trp_doorlocks/close');
        }
    };
    ...
    window.addEventListener("message", function (event) {
        ...
        if (event.data.type == "newDoorSetup") {
            ...
            $("#newDoor").submit(function(event) {                       
                $.post('https://trp_doorlocks/newDoor', JSON.stringify({
        ...
        else
        {
            if (event.data.text == "Locked") {
                event.data.text = '<i style="color:red" class="fas fa-lock"></i>'
                ...
                event.data.text = '<i style="color:limegreen" class="fas fa-lock"></i><br>Locking'
            }
