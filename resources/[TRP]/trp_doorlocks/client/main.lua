...
Citizen.CreateThread(function()
	...
	Citizen.Wait(1000)
	ESX.TriggerServerCallback('trp_doorlocks:getDoorInfo', function(doorInfo)
...
RegisterNetEvent('trp_doorlocks:setState')
AddEventHandler('trp_doorlocks:setState', function(sid, doorID, locked, src)
...
-- `exports.trp_doorlocks:updateDoors()`
...
RegisterCommand('doorlock', function()
	if not isDead and not isCuffed and closestDoor and IsAuthorized(closestV) then
		...
		TriggerServerEvent('trp_doorlocks:updateState', closestDoor, locked, src) -- Broadcast new state of the door to everyone
	end
end)
...
AddEventHandler('esx_lockpick:onUse', function()
	if not isDead and not isCuffed and closestDoor and closestV.lockpick then
		...
		Citizen.Wait(1800)
		local locked = not closestV.locked
		TriggerServerEvent('trp_doorlocks:updateState', closestDoor, locked, nil, true) -- Broadcast new state of the door to everyone
	end
end)
...
RegisterNetEvent('trp_doorlocks:newDoorSetup')
AddEventHandler('trp_doorlocks:newDoorSetup', function(args)
	if not args[1] then
	...
	if doorType == 'door' or doorType == 'sliding' or doorType == 'garage' then
		...
		if arg then pvkey = arg.pvkey end
		TriggerServerEvent('imagineluainjectingforthis:lmfaooooojgfsjg', model, heading, coords, jobs, item, doorLocked, maxDistance, slides, garage, false, doorname, pvkey)
	...
	elseif doorType == 'double' or doorType == 'doublesliding' then
		...
		if arg then doorname = arg.doorname end
		if arg then pvkey = arg.pvkey end
		TriggerServerEvent('imagineluainjectingforthis:lmfaooooojgfsjg', model, heading, coords, jobs, item, doorLocked, maxDistance, slides, garage, true, doorname, pvkey)
		--TriggerServerEvent('trp_doorlocks:newDoorCreate', model, heading, coords, jobs, item, doorLocked, maxDistance, slides, garage, true, doorname)
		print('Successfully sent door data to the server')
		arg = nil
	end
end)
...
RegisterNetEvent('trp_doorlocks:newDoorAdded')
AddEventHandler('trp_doorlocks:newDoorAdded', function(newDoor, doorID, locked)
	Config.DoorList[doorID] = newDoor
	updateDoors()
	TriggerEvent('trp_doorlocks:setState', GetPlayerServerId(PlayerId()), doorID, locked)
end)
