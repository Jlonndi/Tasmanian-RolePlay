...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	ESX.TriggerServerCallback('trp_apartments:getProperties', function(properties)
		Config.Properties = properties
		CreateBlips()
	end)

	ESX.TriggerServerCallback('trp_apartments:getOwnedProperties', function(result)
...
RegisterNetEvent('trp_apartments:sendProperties')
AddEventHandler('trp_apartments:sendProperties', function(properties)
	Config.Properties = properties
	CreateBlips()
...
function EnterProperty(name, owner)
	...
	TriggerServerEvent('trp_apartments:saveLastProperty', name)
...
function ExitProperty(name)
	...
	TriggerServerEvent('trp_apartments:deleteLastProperty')
...
function OpenPropertyMenu(property)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'property', {
		...
		elseif data.current.value == 'leave' then
			TriggerServerEvent('trp_apartments:removeOwnedProperty', property.name)
		elseif data.current.value == 'buy' then
			TriggerServerEvent('trp_apartments:buyProperty', property.name)
		elseif data.current.value == 'rent' then
			TriggerServerEvent('trp_apartments:rentProperty', property.name)
		end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gateway_owned_properties', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gateway_owned_properties_actions', {
			...
			elseif data2.current.value == 'leave' then
				TriggerServerEvent('trp_apartments:removeOwnedProperty', data.current.value)
			end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gateway_available_properties', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gateway_available_properties_actions', {
			...
			if data2.current.value == 'buy' then
				TriggerServerEvent('trp_apartments:buyProperty', data.current.value)
			elseif data2.current.value == 'rent' then
				TriggerServerEvent('trp_apartments:rentProperty', data.current.value)
			end
		end, function(data2, menu2)
			menu2.close()
		end)
...
function OpenRoomMenu(property, owner)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'room', {
		...
		elseif data.current.value == 'player_dressing' then
			...
			ESX.TriggerServerCallback('trp_apartments:getPlayerDressing', function(dressing)
				local elements = {}
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_dressing', {
					...
					TriggerEvent('skinchanger:getSkin', function(skin)
						ESX.TriggerServerCallback('trp_apartments:getPlayerOutfit', function(clothes)
		...
		elseif data.current.value == 'remove_cloth' then
			...
			ESX.TriggerServerCallback('trp_apartments:getPlayerDressing', function(dressing)
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'remove_cloth', {
					...
					menu2.close()
					TriggerServerEvent('trp_apartments:removeOutfit', data2.current.value)
...
function OpenRoomInventoryMenu(property, owner)
	ESX.TriggerServerCallback('trp_apartments:getPropertyInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'room_inventory', {
			...
			if data.current.type == 'item_weapon' then
				...
				TriggerServerEvent('trp_apartments:getItem', owner, data.current.type, data.current.value, data.current.index)
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'get_item_count', {
					...
					if quantity == nil then
						...
						TriggerServerEvent('trp_apartments:getItem', owner, data.current.type, data.current.value, quantity)
						ESX.SetTimeout(300, function()
							OpenRoomInventoryMenu(property, owner)
						end)
					end
...
function OpenPlayerInventoryMenu(property, owner)
	ESX.TriggerServerCallback('trp_apartments:getPlayerInventory', function(inventory)
		local elements = {}

		--if inventoryitem.blackMoney > 0 then
			--table.insert(elements, {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_inventory', {
			...
			if data.current.type == 'item_weapon' then
				menu.close()
				TriggerServerEvent('trp_apartments:putItem', owner, data.current.type, data.current.value, data.current.ammo)
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'put_item_count', {
					...
					if quantity == nil then
						...
						TriggerServerEvent('trp_apartments:putItem', owner, data.current.type, data.current.value, tonumber(data2.value))
						ESX.SetTimeout(300, function()
							OpenPlayerInventoryMenu(property, owner)
						end)
					end
...
AddEventHandler('esx:onPlayerSpawn', function()
	if firstSpawn then
		Citizen.CreateThread(function()
			...
			ESX.TriggerServerCallback('trp_apartments:getLastProperty', function(propertyName)
...
AddEventHandler('trp_apartments:getProperties', function(cb)
	cb(GetProperties())
end)

AddEventHandler('trp_apartments:getProperty', function(name, cb)
	cb(GetProperty(name))
end)

AddEventHandler('trp_apartments:getGateway', function(property, cb)
	cb(GetGateway(property))
end)


RegisterNetEvent('trp_apartments:setPropertyOwned')
AddEventHandler('trp_apartments:setPropertyOwned', function(name, owned, rented)
...
AddEventHandler('trp_apartments:hasEnteredMarker', function(name, part)
	local property = GetProperty(name)
...
AddEventHandler('trp_apartments:hasExitedMarker', function(name, part)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not hasAlreadyEnteredMarker or (isInMarker and (LastProperty ~= currentProperty or LastPart ~= currentPart) ) then
			...
			TriggerEvent('trp_apartments:hasEnteredMarker', currentProperty, currentPart)
		end

		if not isInMarker and hasAlreadyEnteredMarker then
			hasAlreadyEnteredMarker = false
			TriggerEvent('trp_apartments:hasExitedMarker', LastProperty, LastPart)
		end
...
function OpenPropertyInventoryMenu(property, owner)
	ESX.TriggerServerCallback(
		"trp_apartments:getPropertyInventory",
		function(inventory)
			TriggerEvent("trp_inventoryhud:openPropertyInventory", inventory)
		end,
		owner
	)
end