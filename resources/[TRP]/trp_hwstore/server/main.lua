...
MySQL.ready(function()
	MySQL.Async.fetchAll('SELECT * FROM trp_hwstore LEFT JOIN items ON items.name = trp_hwstore.item', {}, function(shopResult)
		for i=1, #shopResult, 1 do
			if shopResult[i].name then
				...
				print(('trp_hwstore: invalid item "%s" found!'):format(shopResult[i].item))
			end
		end
	end)
end)

ESX.RegisterServerCallback('trp_hwstore:requestDBItems', function(source, cb)
	cb(ShopItems)
end)

RegisterServerEvent('trp_hwstore:buyItem')
AddEventHandler('trp_hwstore:buyItem', function(itemName, amount, zone, item)
	...
	if amount < 0 then
		print('trp_hwstore: ' .. xPlayer.identifier .. ' attempted to exploit the shop!')
		return
	end
	...
	if xPlayer.getInventoryItem('money').count >= price then
		...
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'You Need' .. ESX.Math.GroupDigits(missingMoney) .. "$", length = 2500, style = { ['background-color'] = '#2f5c73', ['color'] = '#FFFFFF' } })
	end
end)