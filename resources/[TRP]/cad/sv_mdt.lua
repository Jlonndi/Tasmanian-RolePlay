...
AddEventHandler("mdt:getOffenderDetails", function(offender)
	...
	local result = MySQL.Sync.fetchAll('SELECT * FROM `user_mdt` WHERE `char_id` = @identifier', {
	...
	local warrants = MySQL.Sync.fetchAll('SELECT * FROM `mdt_warrants` WHERE `char_id` = @identifier', {
		['@identifier'] = offender.identifier
	})
	if warrants[1] then
...
AddEventHandler("mdt:saveOffenderChanges", function(identifier, changes, identifier)
	local usource = source
	MySQL.Async.fetchAll('SELECT * FROM `user_mdt` WHERE `char_id` = @identifier', {
		['@identifier']  = identifier
	}, function(result)
		if result[1] then
			MySQL.Async.execute('UPDATE `user_mdt` SET `notes` = @notes, `mugshot_url` = @mugshot_url, `bail` = @bail WHERE `char_id` = @identifier', {
				['@identifier'] = identifier,
				['@notes'] = changes.notes,
			...
			MySQL.Async.insert('INSERT INTO `user_mdt` (`char_id`, `notes`, `mugshot_url`, `bail`) VALUES (@identifier, @notes, @mugshot_url, @bail)', {
				['@identifier'] = identifier,
				['@notes'] = changes.notes,
				['@mugshot_url'] = changes.mugshot_url,
				['@bail'] = changes.bail
...
AddEventHandler("mdt:saveReportChanges", function(data)
	MySQL.Async.execute('UPDATE `mdt_reports` SET `title` = @title, `incident` = @incident WHERE `id` = @identifier', {
...
AddEventHandler("mdt:deleteReport", function(identifier)
	MySQL.Async.execute('DELETE FROM `mdt_reports` WHERE `id` = @identifier', {
		['@identifier']  = identifier
	})
...
AddEventHandler("mdt:submitNewReport", function(data)
	...
	MySQL.Async.insert('INSERT INTO `mdt_reports` (`char_id`, `title`, `incident`, `charges`, `author`, `name`, `date`) VALUES (@identifier, @title, @incident, @charges, @author, @name, @date)', {
		...
		TriggerEvent("mdt:getReportDetailsById", identifier, usource)
...
AddEventHandler("mdt:getVehicle", function(vehicle)
	...
	local bail = MySQL.Sync.fetchAll('SELECT `bail` FROM user_mdt WHERE `char_id` = @identifier', {
		['@identifier'] = vehicle.owner_id
	})
...
AddEventHandler("mdt:deleteWarrant", function(identifier)
	local usource = source
	MySQL.Async.execute('DELETE FROM `mdt_warrants` WHERE `id` = @identifier', {
		['@identifier']  = identifier
	}, function()
		TriggerClientEvent("mdt:completedWarrantAction", usource)
	end)
