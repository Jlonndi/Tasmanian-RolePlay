...
Citizen.CreateThread(function()
	...
	while OPCore.Functions.GetPlayerData().job == nil do
		Citizen.Wait(10)
	end
    PlayerData = OPCore.Functions.GetPlayerData()
    
end)

--------------------------------------------onload player----------------------------
RegisterNetEvent('OPCore:Client:OnPlayerLoaded')
AddEventHandler('OPCore:Client:OnPlayerLoaded', function()
    OPCore.Functions.GetPlayerData(function(PlayerData)
...
-------------------------------------------setjob-------------------------------------
RegisterNetEvent('OPCore:Client:OnJobUpdate')
AddEventHandler('OPCore:Client:OnJobUpdate', function(JobInfo)
...
----------------------------------------------------NOT USE work in progress in the next update---------------------------------
--[[
...
AddEventHandler('loffe_robbery:msgPolice', function(store, robber)
   -- local mugshot, mugshotStr = ESX.Game.GetPedMugshot(GetPlayerPed(GetPlayerFromServerId(robber)))
    --exports['mythic_notify']:DoHudText('inform', Config.Shops[store].name, Translation[Config.Locale]['robbery'], 8000)
    --OPCore.Functions.Notify('Purchased', "success", 7000)
...
------------------------------LOCAL BLIP------------------------------------------------
local blips = {
    {title="Robbery market", colour=16, id=156, x = 1134.02, y = -983.34, z = 45.42},
    {title="Robbery market", colour=16, id=156, x = 1392.63, y = 3606.41, z = 34.98},
    {title="Robbery market", colour=16, id=156, x = 372.22, y = 326.68, z = 102.57},
    {title="Robbery market", colour=16, id=156, x = 24.38, y = -1346.91, z = 28.5},
    {title="Robbery market", colour=16, id=156, x = -1221.71, y = -908.53, z = 12.33}
    --
                    ...
                    if HasEntityClearLosToEntityInFront(me, peds[i], 19) and not IsPedDeadOrDying(peds[i]) and GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 5.0 then
                        if not robbing then
                            local canRob = nil
                                ...
                                ---------------------Enter the police notification call----------------
                                ---------------
                                --TriggerServerEvent('alarm:server:PoliceAlertMessage', "Suspicious situation", pos, true)
                                ---Enter the police notification call
                                ----------------
                                TriggerServerEvent('police:server:PoliceAlertMessage', 'robbery Market', 'Coord '..plyCoords.z..' '..plyCoords.z..' '..plyCoords.z..'', '')
