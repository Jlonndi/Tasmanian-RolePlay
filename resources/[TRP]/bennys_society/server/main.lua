...
RegisterNetEvent('esx_bennysjob:impound')
AddEventHandler('esx_bennysjob:impound', function(targetCar)
	local plate = targetCar
		...
		if responses[1] then
			MySQL.Async.execute('UPDATE owned_vehicles SET impound = @impound WHERE plate = @plate', {
				...
				if rowsChanged == 0 then
					...
					--print("Vehicle with plate "..plate.." is now impounded")
				end
			end)
		else
			--print("Local Vehicle has been impounded")
		end
	end)
end)