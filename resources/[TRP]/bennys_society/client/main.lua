...
local CurrentAction, CurrentActionMsg, CurrentActionData, currentTask = nil, '', {}, {}
...
function OpenbennysHarvestMenu()
	if Config.EnablePlayerManagement and ESX.PlayerData.job and ESX.PlayerData.job.grade_name ~= 'recrue' then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'bennys_harvest', {
			title    = _U('harvest'),
			align    = 'top-left',
			elements = elements
		}, function(data, menu)
...
function OpenbennysCraftMenu()
	if Config.EnablePlayerManagement and ESX.PlayerData.job and ESX.PlayerData.job.grade_name ~= 'recrue' then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'bennys_craft', {
			title    = _U('craft'),
			align    = 'top-left',
			elements = elements
		}, function(data, menu)
...
function OpenMobilebennysActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_bennys_actions', {
		title    = _U('bennys'),
		align    = 'top-left',
		elements = {
		...
		elseif data.current.value == 'del_vehicle' then
			...
			if DoesEntityExist(vehicle) then
				...
				currentTask.task = ESX.SetTimeout(10000, function()
					...
					TriggerServerEvent('esx_bennysjobjob:impound', vehicleData.plate)
		...
		elseif data.current.value == 'object_spawner' then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_bennys_actions_spawn', {
				title    = _U('objects'),
				align    = 'top-left',
				elements = {
...
Citizen.CreateThread(function()
	...
	AddTextComponentSubstringPlayerName(_U('bennys'))
	...
	while true do
		...
		if closestDistance ~= -1 and closestDistance <= 3.0 then
			if LastEntity ~= closestEntity then
				TriggerEvent('esx_bennysjob:hasEnteredEntityZone', closestEntity)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, 178) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'bennys' then
			if NPCOnJob then
