...
RegisterNetEvent('golf:beginGolf')
AddEventHandler('golf:beginGolf', function()
	exports['mythic_notify']:SendAlert('success', 'Payment Successful')
	--startGolf()
	TriggerEvent("golf:spawnCart")
	TriggerEvent("golf:startHud", source)
	TriggerEvent("golf:startGolf")
	--spawnCart()
	TriggerEvent("customNotification","Press E to swing, A-D to rotate, Y to swap club.")
	Citizen.Trace("Payment Successful This is Fine")
end)
RegisterNetEvent('golf:beginGolfnomoney')
AddEventHandler('golf:beginGolfnomoney', function()
	--exports['mythic_notify']:SendAlert('error', 'no money')
	--startGolf()
	
	exports['mythic_notify']:SendAlert('error', 'Payment Failed Not enough money requires $500')
	--Citizen.Trace("1")
	endgame()
	
end)

RegisterNetEvent('golf:beginGolfHud')
AddEventHandler('golf:beginGolfHud', function()
	--startGolfHud()
	...
	while true do
		...
		if distance < 1.5 then
			...
			if golfplaying then
				...
				DisplayHelpText("Press ~g~~INPUT_CONTEXT~~s~ to start golf ($500).")
			end
			if (IsControlJustReleased(1, 38)) then
				...
				if (golfplaying) then
					...
					Citizen.Trace("This is fine")
					TriggerServerEvent("golf:payment")
					
					 -- If you plan to have it cost money, you need to remove this and only call it when they paid
					--exports['mythic_notify']:SendAlert('success', 'Payment Successful')
					--Citizen.Trace("afterpayment function")
...
RegisterNetEvent('golf:spawnCart')
AddEventHandler('golf:spawnCart', function()
...
RegisterNetEvent('golf:startHud')
AddEventHandler('golf:startHud', function()
	while golfplaying do
		...
		if golfhole ~= 0 then
			...
			drawGolfTxt(0.9193, 1.391, 1.0,2.8,0.4, "~s~" .. golfstrokes .. "~r~ - ~s~" .. totalgolfstrokes .. "~r~ - ~s~" .. clubname .. "~r~ - ~s~" .. distance .. " m", 255, 255, 255, 255)
		end
	end
end)

RegisterNetEvent('golf:startGolf')
AddEventHandler('golf:startGolf', function()
	Citizen.Trace("--Start Golf Client")
	--endgame()
	...
	golfplaying = true
	Citizen.Trace("3")
	TriggerEvent("beginGolfHud", source)
	Citizen.Trace("4")
	Citizen.CreateThread(function()
		while golfplaying do
			Citizen.Wait(1000)
...
function endShot()
	...
	if distance < 1.5 then
		...
		exports['mythic_notify']:SendAlert('Success', 'You got the ball with in range!')
	...
	if golfstrokes > 12 then
		...
		exports['mythic_notify']:SendAlert('error', 'You took too many shots..')
		exports['mythic_notify']:SendAlert('error', 'Move to the next hole')
		totalgolfstrokes = golfstrokes + totalgolfstrokes
		golfstrokes = 0
		ballstate = 1
	end
