...
Citizen.CreateThread(function()
	...
	TriggerServerEvent('trp_licenseshop:ServerLoadLicenses')
end)
...
RegisterNetEvent('trp_licenseshop:loadLicenses')
AddEventHandler('trp_licenseshop:loadLicenses', function(licenses)
	Licenses = licenses
end)
...
function OpenLicenseShop()
	...
	if Config.RequireDMV then
		...
		if Config.WeaponShop then
			...
			if ESX.PlayerData.job.grade >= 0 then 
			    table.insert(elements, {label = _U('cuffs') .. ' <span style="color: green;"></span>', value = 'freebuy_cuffs'})
				table.insert(elements, {label = _U('TAZER') .. ' <span style="color: green;"></span>', value = 'TAZER'})
			if ESX.PlayerData.job.grade >= 1 then 
			    table.insert(elements, {label = _U('ammo_pistol') .. ' <span style="color: green;"></span>', value = 'freebuy_ammo'})
			    table.insert(elements, {label = _U('combat_pistol') .. ' <span style="color: green;"></span>', value = 'combat_pistol'})
			if ESX.PlayerData.job.grade >= 2 then 
				table.insert(elements, {label = 'POLICE Carbine Rifle'.. ' <span style="color: green;"></span>', value = 'carbine_rifle'})
			end
		end
		end
			end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'license_shop_actions', {
		...
		elseif data.current.value == 'buy_ammo' then
			TriggerServerEvent('esx_licenseshop:buyPistolAmmo')
			IsInMainMenu = false
			menu.close()
		elseif data.current.value == 'freebuy_ammo' then
			TriggerServerEvent('trp:buyPistolAmmo')
			IsInMainMenu = false
			--menu.close()
		elseif data.current.value == 'combat_pistol' then
			TriggerServerEvent('trp:policecombatpistol')
			IsInMainMenu = false
			--menu.close()
		elseif data.current.value == 'TAZER' then
			TriggerServerEvent('trp:TAZER')
			IsInMainMenu = false
			--menu.close()
		elseif data.current.value == 'carbine_rifle' then
			TriggerServerEvent('trp:carbine_rifle')
			IsInMainMenu = false
			--menu.close()
		elseif data.current.value == 'freebuy_cuffs' then
			TriggerServerEvent('trp:buycuffs')
			IsInMainMenu = false
			--menu.close()
		end
...
-- Entered Marker
AddEventHandler('trp_licenseshop:hasEnteredMarker', function(zone)
	CurrentAction = 'license_menu'
	CurrentActionMsg = _U('press_access')
	CurrentActionData = {}
end)
...
-- Exited Marker
AddEventHandler('trp_licenseshop:hasExitedMarker', function(zone)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('trp_licenseshop:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('trp_licenseshop:hasExitedMarker', LastZone)
		end
