...
local firstname = 'John Developer'
...
RegisterNetEvent('trp:core:updateName')
AddEventHandler('trp:core:updateName', function(job)   
    ESX.TriggerServerCallback("trp:core:getNames", function(FullName)
		firstname = FullName
		lastname  = ''
	end)
...
AddEventHandler("trp_inventoryhud:hotbarEquipt", function(slot)
  SendNUIMessage({
    action = "getSlotItem",
    slot = slot
  })
...
AddEventHandler("trp_inventoryhud:hotbarClear", function()
  SendNUIMessage({
    action = "clearHotbar"
  })
end)
...
RegisterKeyMapping('InventoryOpen', 'Open Inventory', 'keyboard', 'Tab')

local KeyCommand = {
	HotBar1 = {
		Command = 'InventoryHotBar1',
        Key = '1'
	},	
    HotBar2 = {
		Command = 'InventoryHotBar2',
        Key = '2'
	},
    HotBar3 = {
		Command = 'InventoryHotBar3',
        Key = '3'
	},
    HotBar4 = {
		Command = 'InventoryHotBar4',
        Key = '4'
	},
    HotBar5 = {
		Command = 'InventoryHotBar5',
        Key = '5'
	},
    HotBar6 = {
		Command = 'InventoryHotBar6',
        Key = '6'
	},
    HotBar7 = {
		Command = 'InventoryHotBar7',
        Key = '7'
	},
    HotBar8 = {
		Command = 'InventoryHotBar8',
        Key = '8'
	},
    HotBar9 = {
		Command = 'InventoryHotBar9',
        Key = '9'
	},
}

Citizen.CreateThread(function()
    for k,v in pairs(KeyCommand) do
        RegisterKeyMapping(v.Command, v.Command, 'keyboard', v.Key)
        RegisterCommand(v.Command, function()
            if IsInputDisabled(0) and not IsDead then
                TriggerEvent('trp_inventoryhud:hotbarEquipt', v.Key)
            end
        end, false)
    end
    Citizen.Wait(10000)
...
RegisterNetEvent("trp_inventoryhud:closeInventory")

AddEventHandler("trp_inventoryhud:closeInventory", function()
...
function useItem(item, inInventory)
  ...
  if GetSelectedPedWeapon(PlayerPedId()) == GetHashKey(item) then
    ...
    TriggerServerEvent('trp-Inventory:UseInventoryItem', "WEAPON_UNARMED")
  elseif endsWith(item, "WEAPON") then

    SetCurrentPedWeapon(PlayerPedId(), GetHashKey(item), true)
    TriggerServerEvent('trp-Inventory:UseInventoryItem', item)
    if inInventory then
...
RegisterNUICallback(
    ...
    function(data, cb)
        ...
        if type(data.number) == "number" and math.floor(data.number) == data.number then
            ...
            TriggerServerEvent('trp-Inventory:DestroyingPussy', data.number, data.item.name)
    ...
    function(data, cb, xPlayer)
        ...
        if foundPlayer then
			...
			if data.item.type == "item_money" then
                ...
                TriggerServerEvent('trp-Inventory:Givingdickdoodle', data.player, count, data.item.name)
                
                -- function booleans calls for 4 function booleens in Givingdickdoodle read comments to understand
                --TriggerServerEvent('trp-Inventory:Givingdickdoodle', count, data.item.name)
                ...
                --TriggerServerEvent('trp-Inventory:Givingdickdoodle', "Gave item", data.item.type, data.item.name, count)
            end
            cb("ok")
